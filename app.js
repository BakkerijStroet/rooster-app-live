...
const myHoursChoiceTabs = document.getElementById("myHoursChoiceTabs");
...
const smartPlanningProposalSummary = document.getElementById("smartPlanningProposalSummary");
const smartPlanningProposalList = document.getElementById("smartPlanningProposalList");
const smartPlanningChecksSummary = document.getElementById("smartPlanningChecksSummary");
const smartPlanningChecksList = document.getElementById("smartPlanningChecksList");
...
const approvalPreviousWeekButton = document.getElementById("approvalPreviousWeekButton");
const approvalCurrentWeekButton = document.getElementById("approvalCurrentWeekButton");
const approvalNextWeekButton = document.getElementById("approvalNextWeekButton");
...
function setMobileHoursFeedback(workLogId, text = "", type = "info") {
  activeMobileHoursFeedback = {
    workLogId: workLogId || "",
    text: text || "",
    type: type || "info"
  };
  ...
  if (feedbackElement) {
    ...
    setClassName(feedbackElement, `mobile-hours-feedback ${text ? `is-${type || "info"}` : "is-empty"}`);
  }
}
...
function getMobileWorkLogSubmitResult(workLogId) {
  const workLog = workLogs.find((log) => log.id === workLogId) || null;
  ...
  if (workLog?.status === "approved") {
    return {
      isSubmitted: true,
      message: "Uren zijn al goedgekeurd"
    };
  }

  return {
    isSubmitted: false,
    message: "Uren zijn nog niet ingediend"
  };
}
...
function renderWorkLogCardMarkup(entry, workLog = getWorkLogForEntry(entry), options = {}) {
  ...
  const mobileStatusLabel = !workLog
    ? "Nog invullen"
    : workLog.status === "approved"
      ? "Goedgekeurd"
      : workLog.status === "open"
        ? "Ingediend"
        : (workLog.status === "rejected" || workLog.status === "revision")
          ? "Opmerking"
          : "Opgeslagen";
  ...
  return `
    <article class="hours-registration-card ${workLog ? "is-saved" : ""} ${hasDeviation ? "has-deviation" : ""} ${entry.isManualHours ? "is-manual-hours" : ""} ${isMobileEntryActive ? "is-mobile-entry-active" : ""} status-${statusClass}" data-worklog-card-id="${workLogId}">
      ...
      <div class="mobile-hours-entry-card">
        <div class="mobile-hours-entry-main">
          <span class="mobile-hours-day">${formatWeekday(entry.day)} ${formatDate(entry.day)}</span>
          <strong>${shiftName}</strong>
          <span>${mobilePlannedTimeLabel}</span>
        </div>
        <span class="mobile-hours-status status-${statusClass === "draft" ? "open" : statusClass === "revision" ? "rejected" : statusClass}">${mobileStatusLabel}</span>
        ${(workLog?.notes || workLog?.managerNote || workLog?.employeeReply)
          ? `<span class="mobile-hours-note">Opmerking</span>`
          : ""}
        ...
        ${isInputLockedWithFuture
          ...
          : `<button type="button" class="mobile-hours-open-button" data-mobile-worklog-open="${workLogId}">
              ${isMobileEntryActive ? "Nu invullen" : "Uren invullen"}
            </button>`}
      </div>
...
function renderMyHoursChoiceTabs(activeChoice = "today") {
  const activeTab = activeChoice === "worked-detail"
    ? "worked"
    : (activeChoice === "fill" || activeChoice === "missing" || activeChoice === "extra" ? "today" : activeChoice);
  const choices = [
    { value: "today", label: "Uren doorgeven" },
    { value: "worked", label: "Mijn gewerkte uren" }
  ];

  return choices.map((choice) => `
    <button type="button" class="request-switch-button ${activeTab === choice.value ? "active" : ""}" data-my-hours-choice="${choice.value}">
      ${choice.label}
    </button>
  `).join("");
}

function isEmployeeWorkLogComplete(workLog) {
  return workLog?.status === "open" || workLog?.status === "approved";
}
...
function maybeShowOpenRequestReminder() {
  ...
  if (isPlannerRole()) {
    ...
    if (totalOpen && pendingHoursCount) {
      showMessage("Er staan open aanvragen in de Roosterapp. Er staan uren klaar om te controleren in de Roosterapp.", "warning");
      return;
    }
    ...
    showMessage("Er staan uren klaar om te controleren in de Roosterapp.", "warning");
    return;
  }
...
function renderSchedulePlanningOverview() {
  ...
  const weekDates = getWeekDates(selectedWeek);
  const openCount = Number.isFinite(week.openCount) ? week.openCount : 0;
  const filledCount = Array.isArray(week.weekEntries) ? week.weekEntries.length : 0;
  ...
  if (planningOverviewSummary) {
    ...
    planningOverviewSummary.innerHTML = `
      ...
      <article class="planning-overview-summary-card ${openCount > 0 ? "is-warning" : "is-complete"}">
        <strong>Open</strong>
        <span>${openCount}</span>
      </article>
      <article class="planning-overview-summary-card ${week.contractImbalanceCount > 0 ? "is-warning" : "is-complete"}">
        <strong>Contract</strong>
        <span>${week.contractImbalanceCount ? `${week.contractImbalanceCount} aandacht` : "Rustig"}</span>
      </article>
    `;
  }
  ...
  setClassName(planningOverviewList, "planning-overview-list");
  planningOverviewList.innerHTML = `
...
function getSmartPlanningSelectedWeek() {
  const weekValue = smartPlanningWeekInput?.value || weekInput?.value || weekFilterInput?.value || getCurrentWeekValue();
...
function renderSmartPlanningDemandOverview(data = getSmartPlanningWeekData()) {
  ...
  data.openItems.forEach((item) => {
    if (!openByDay.has(item.day)) {
      openByDay.set(item.day, []);
    }
    openByDay.get(item.day).push(item.shift);
  });

  if (smartPlanningDemandSummary) {
    smartPlanningDemandSummary.textContent = `${formatWeekLabel(data.selectedWeek)} · ${data.openItems.length} open diensten`;
  }

  setClassName(smartPlanningDemandList, "smart-planning-list");
  smartPlanningDemandList.innerHTML = data.weekDates.map((day) => {
    const openShifts = openByDay.get(day) || [];
    const dayEntries = data.weekEntries.filter((entry) => entry.day === day);

    return `
      <article class="smart-planning-day-row ${openShifts.length ? "has-open" : ""}">
        <div>
          <strong>${formatWeekday(day)}</strong>
          <span>${formatDate(day)}</span>
        </div>
        <div class="smart-planning-day-metrics">
          <span>${dayEntries.length} ingevuld</span>
          <span>${openShifts.length} open</span>
        </div>
        ${openShifts.length ? `
          <div class="smart-planning-tags">
            ${openShifts.slice(0, 5).map((shift) => `<span>${getCompactRosterShiftLabel(shift.name)} ${shift.startTime || ""}-${shift.endTime || ""}</span>`).join("")}
            ${openShifts.length > 5 ? `<span>+${openShifts.length - 5}</span>` : ""}
          </div>
        ` : `<div class="panel-note">Geen open diensten.</div>`}
      </article>
    `;
  }).join("");
}
      ...
      ${data.weekDates.map((day) => {
        ...
        return `
          <article class="planning-day-card week-roster-day-card smart-planning-proposal-day ${hasOpenShifts ? "is-open-day" : "is-closed"}">
            ...
            <div class="roster-groups planning-roster-groups">
              ${dayRows.length
                ? `${renderSmartPlanningProposalRosterGroup("Bakkerij", groupedRows.bakery, "bakery")}${renderSmartPlanningProposalRosterGroup("Winkel", groupedRows.shop, "shop")}`
                : `<div class="planning-shift-empty">Geen diensten</div>`}
            </div>
          </article>
        `;
...
function renderSmartPlanningProposalRosterGroup(title, groupRows, groupType) {
  if (!groupRows.length) {
    return "";
  }
...
function renderSmartPlanningSelectedShiftPanel() {
  ...
  return `
    <aside class="smart-planning-selected-shift-panel">
      <div class="smart-planning-selected-shift-head">
        <div>
          <strong>${formatWeekday(selectedItem.day)} · ${formatDate(selectedItem.day)}</strong>
          <span>${departmentLabel} · ${getCompactRosterShiftLabel(selectedItem.shiftName)} · ${shiftTime}</span>
        </div>
...
function renderSmartPlanningPanel() {
  ...
  const selectedWeek = getSmartPlanningSelectedWeek();
  if (!smartPlanningWeekInput.value) {
...
function applyRoleUI() {
  ...
  if (switchUserButton) {
    ...
    switchUserButton.setAttribute("aria-hidden", isAuthenticated ? "false" : "true");
  }
...
function renderMyHours() {
  ...
  if (!employeeName) {
    ...
    if (myHoursChoiceTabs) {
      myHoursChoiceTabs.hidden = true;
      myHoursChoiceTabs.innerHTML = "";
    }
    if (submitWeekHoursButton) {
  ...
  const missingPastEntries = employeeEntries
    .filter((entry) => {
      if (entry.day >= todayValue) {
        return false;
      }

      const workLog = getWorkLogForEntry(entry);
      return !isEmployeeWorkLogComplete(workLog);
    })
    .sort((entryA, entryB) => entryA.day.localeCompare(entryB.day) || entryA.startTime.localeCompare(entryB.startTime));
  ...
  if (!isPlannerRole()) {
    ...
    const resolvedChoice = activeMyHoursChoice === "extra" ? "today" : (activeMyHoursChoice || "today");
    ...
    if (myHoursChoiceTabs) {
      myHoursChoiceTabs.hidden = false;
      myHoursChoiceTabs.innerHTML = renderMyHoursChoiceTabs(resolvedChoice);
    }
    ...
    if (resolvedSection === "today") {
          ...
          <section class="my-hours-open-block">
            ...
            <div class="hours-missing-list">
              ${missingPastEntries.map((entry) => {
                ...
                return `
                  <button type="button" class="hours-missing-item" data-hours-pick-date="${entry.day}">
                    <span>${formatWeekday(entry.day)} ${formatDate(entry.day)} · ${entry.shiftName || "Dienst"} · ${entry.startTime || ""}${entry.endTime ? `-${entry.endTime}` : ""}</span>
      ...
      myHoursRegistrations.innerHTML = `
        ${todayHoursMarkup}
        ${openPastHoursMarkup}
        <section class="my-hours-extra-block">
          <div class="my-hours-open-head">
            <span class="hours-section-kicker">Extra uren</span>
            <strong>Extra uren doorgeven</strong>
          </div>
    ...
    activeMyHoursEntryMode = "planned";
    setClassName(myHoursSummary, "totals");
    const entryModeMarkup = missingPastDays.length
      ? `
  ...
  if (!isPlannerRole() && existingLog && (existingLog.status === "open" || existingLog.status === "approved")) {
    setMobileHoursFeedback(
      ...
      existingLog.status === "approved"
        ? "Goedgekeurde uren staan vast."
        : "Deze uren zijn al ingediend.",
      "error"
    );
    showMessage(
      existingLog.status === "approved"
...
function getHoursControlStatusClass(log) {
  ...
  if (log?.status === "rejected" || log?.status === "revision") {
    return "rejected";
  }
...
function getHoursControlSummary({ allWeekLogs, openLogs, approvedLogs, reviewLogs }) {
  return {
    open: openLogs.length,
    approved: approvedLogs.length,
    review: reviewLogs.length,
    total: allWeekLogs.length
  };
}

function sortPlannerHoursControlItems(logA, logB) {
  const priorityMap = { open: 0, revision: 1, rejected: 2, draft: 3, approved: 4 };
  const priorityCompare = (priorityMap[logA.status] ?? 9) - (priorityMap[logB.status] ?? 9);

  if (priorityCompare !== 0) {
    return priorityCompare;
  }

  return logA.day.localeCompare(logB.day) ||
    logA.employeeName.localeCompare(logB.employeeName, "nl") ||
    logA.plannedStart.localeCompare(logB.plannedStart);
}

function getPlannerHoursControlItems({ allWeekLogs, openLogs, approvedLogs, reviewLogs }) {
  if (activeHoursApprovalFilter === "approved") {
    return approvedLogs.slice().sort(sortPlannerHoursControlItems);
  }

  if (activeHoursApprovalFilter === "open") {
    return openLogs.slice().sort(sortPlannerHoursControlItems);
  }

  if (activeHoursApprovalFilter === "review") {
    return reviewLogs.slice().sort(sortPlannerHoursControlItems);
  }

  return allWeekLogs.slice().sort(sortPlannerHoursControlItems);
}

function renderHoursControlSummaryCards(summary) {
  const cards = [
    { key: "open", label: "Nog te controleren", value: summary.open },
    { key: "approved", label: "Goedgekeurd", value: summary.approved },
    { key: "review", label: "Afgewezen / gecorrigeerd", value: summary.review },
    { key: "all", label: "Totaal deze week", value: summary.total }
  ];

  return `
    <div class="hours-control-summary-row">
      ${cards.map((card) => `
        <button
          type="button"
          class="hours-control-summary-card ${activeHoursApprovalFilter === card.key ? "active" : ""}"
          data-hours-approval-filter="${card.key}"
          aria-pressed="${activeHoursApprovalFilter === card.key ? "true" : "false"}"
        >
...
function renderHoursControlFilterButtons() {
  const buttons = [
    { key: "all", label: "Alles" },
    { key: "open", label: "Nog te controleren" },
    { key: "approved", label: "Goedgekeurd" },
    { key: "review", label: "Afgewezen / correctie" },
    { key: "missing", label: "Ontbrekend" }
  ];

  return `
    <div class="form-actions compact-actions hours-control-filter-row">
...
function renderHoursControlWeekStatus({ selectedWeek, selectedEmployee, weekReviewState, summary, openLogs }) {
  const bulkApproveButtonMarkup = openLogs.length
    ? `
      <div class="form-actions compact-actions hours-control-bulk-actions">
        <button type="button" data-worklog-bulk-approve="selected-week" data-worklog-week="${selectedWeek}" data-worklog-employee="${selectedEmployee}" onclick='if (confirm("Alle open uren van deze week goedkeuren?")) approveWorkLogsForWeek(${JSON.stringify(selectedWeek)}, ${JSON.stringify(selectedEmployee || "")});'>Alles goedkeuren</button>
      </div>
    `
    : "";

  return `
    <article class="hours-control-status-card">
      <div class="hours-control-status-head">
        <div>
          <strong>Weekstatus</strong>
      ...
      ${renderHoursControlSummaryCards(summary)}
      <div class="hours-control-status-footer">
        <span>${weekReviewState.note || "Controleer de ingediende uren van deze week."}</span>
        ${bulkApproveButtonMarkup}
      </div>
      ${renderHoursControlFilterButtons()}
    </article>
  `;
}

function renderHoursControlMissingDays({ selectedWeek, selectedEmployee, weekReviewState, summary, openLogs, missingDays }) {
  setClassName(hoursApprovalQueue, "hours-control-list");
  hoursApprovalQueue.innerHTML = `
    ${renderHoursControlWeekStatus({ selectedWeek, selectedEmployee, weekReviewState, summary, openLogs })}
    <article class="hours-control-card">
      <div class="hours-control-list-title">
        <strong>Ontbrekende dagen</strong>
        <span>${selectedEmployee || "Alle medewerkers"} · ${selectedWeek.replace("-W", " week ")}</span>
      </div>
      ${missingDays.length ? `
        <div class="hours-control-items">
          ${missingDays.map((day) => `
            <article class="hours-control-item is-rejected">
              <div class="hours-control-item-grid">
                <div class="hours-control-date"><strong>${formatWeekday(day)}</strong><span>${formatDate(day)}</span></div>
                <div class="hours-control-main"><strong>Ontbrekende uren</strong><span>Voor deze dag ontbreken nog urenregistraties.</span></div>
                <span class="status-pill status-rejected">Ontbrekend</span>
              </div>
            </article>
          `).join("")}
        </div>
      ` : `<div class="hours-control-empty">Geen uren gevonden voor deze selectie.</div>`}
    </article>
  `;
}

function renderPlannerHoursControlItem(log) {
  const statusClass = getHoursControlStatusClass(log);
  const workedHours = getWorkedHoursFromLog(log);
  const plannedText = log.plannedStart && log.plannedEnd ? `${log.plannedStart} - ${log.plannedEnd}` : "Niet gepland";
  const actualText = log.actualStart && log.actualEnd ? `${log.actualStart} - ${log.actualEnd}` : "Nog niet ingevuld";
  const pauseText = `${Number(log.breakMinutes) || 0} min`;
  const totalText = workedHours !== null ? formatHours(workedHours) : "-";
  const hasDeviation = log.actualStart !== log.plannedStart || log.actualEnd !== log.plannedEnd || Number(log.breakMinutes) > 0 || Boolean(log.notes?.trim());
  const isApprovedLog = log.status === "approved";

  return `
    <article class="hours-control-item is-${statusClass} ${hasDeviation ? "has-deviation" : ""}">
      <div class="hours-control-item-grid">
        <div class="hours-control-date">
          <strong>${formatWeekday(log.day)}</strong>
          <span>${formatDate(log.day)}</span>
        </div>
        <div class="hours-control-main">
          <strong>${log.employeeName}</strong>
          <span>${log.shiftName || "Urenregistratie"}</span>
        </div>
        <div class="hours-control-meta"><span>Gepland</span><strong>${plannedText}</strong></div>
        <div class="hours-control-meta"><span>Opgegeven</span><strong>${actualText}</strong></div>
        <div class="hours-control-meta"><span>Pauze</span><strong>${pauseText}</strong></div>
        ...
        <div class="hours-control-note-area">
          ${log.notes ? `<span>Medewerker: ${log.notes}</span>` : ""}
          ${log.employeeReply ? `<span>Reactie: ${log.employeeReply}</span>` : ""}
          ${isApprovedLog ? (log.managerNote ? `<span>Opmerking: ${log.managerNote}</span>` : `<span>Goedgekeurd</span>`) : `
            <label class="hours-control-note-field">
              <span>Korte toelichting</span>
              <input type="text" maxlength="200" data-worklog-review-note="${log.id}" value="${log.managerNote || ""}" placeholder="Korte toelichting">
            </label>
          `}
        </div>
        <div class="actions compact-actions hours-control-actions">
          ${isApprovedLog ? `
            <span class="hours-registration-audit">Geen actie nodig</span>
          ` : `
            ...
            <button type="button" class="warning" data-worklog-review="rejected" data-worklog-id="${log.id}">Afwijzen</button>
          `}
        </div>
      </div>
    </article>
...
function renderHoursApproval() {
  ...
  hoursApprovalQueue.innerHTML = `
    ...
    <article class="hours-control-card">
      <div class="hours-control-list-title">
        <strong>${activeHoursApprovalFilter === "approved" ? "Goedgekeurde uren" : activeHoursApprovalFilter === "open" ? "Nog te controleren uren" : activeHoursApprovalFilter === "review" ? "Afgewezen / correctie" : "Uren van deze week"}</strong>
        <span>${filteredLogs.length} ${filteredLogs.length === 1 ? "regel" : "regels"} · ${selectedEmployee || "Alle medewerkers"}</span>
      </div>
      <div class="hours-control-table-head" aria-hidden="true">
        <span>Datum</span>
        <span>Medewerker / dienst</span>
        <span>Gepland</span>
        <span>Opgegeven</span>
        <span>Pauze</span>
        <span>Totaal</span>
      ...
      <div class="hours-control-items">
        ${filteredLogs.map(renderPlannerHoursControlItem).join("")}
      </div>
    </article>
  `;
}
...
myHoursRegistrations?.addEventListener("click", (event) => {
  ...
  if (pickDateButton?.dataset.hoursPickDate && !isPlannerRole() && hoursDateInput) {
    const targetDate = pickDateButton.dataset.hoursPickDate;
  ...
  if (mobileOpenButton?.dataset.mobileWorklogOpen) {
    ...
    setMobileHoursFeedback(activeMobileWorkLogId, "Vul je uren in en kies Opslaan of Indienen.", "info");
    renderMyHours();
    focusMobileWorkLogCard(activeMobileWorkLogId);
    return;
  }

  const mobileCancelButton = event.target.closest("[data-mobile-worklog-cancel]");

  if (mobileCancelButton?.dataset.mobileWorklogCancel) {
    activeMobileWorkLogId = "";
    setMobileHoursFeedback(mobileCancelButton.dataset.mobileWorklogCancel, "Invullen geannuleerd.", "info");
    renderMyHours();
    return;
  }
  ...
  if (workLogAction === "planned" && workLogId) {
    const contextEntry = getWorkLogContextById(workLogId);
    ...
    savePreferences();
    refreshWorkLogValidationForCard(workLogId);
  ...
  if ((workLogAction === "save" || workLogAction === "submit") && workLogId) {
    ...
    if (isMobileSubmitAction) {
      ...
      const submitResult = getMobileWorkLogSubmitResult(workLogId);

      if (submitResult.isSubmitted) {
        activeMobileWorkLogId = "";
        setMobileHoursFeedback(workLogId, submitResult.message, "success");
        renderMyHours();
        focusMobileWorkLogCard(workLogId);
      } else {
        activeMobileWorkLogId = workLogId;
...
myHoursChoiceTabs?.addEventListener("click", (event) => {
  ...
  if (choiceButton?.dataset.myHoursChoice && !isPlannerRole()) {
    activeMyHoursChoice = choiceButton.dataset.myHoursChoice === "extra" ? "today" : choiceButton.dataset.myHoursChoice;
...
planningOverviewMonthInput?.addEventListener("change", () => {
  syncSelectedWeekControls(getSelectedSchedulePlanningWeek());
  renderSchedulePlanningOverview();
});
...
planningOverviewList?.addEventListener("click", (event) => {
  ...
  if (editButton) {
    ...
    const shiftKey = `${day}|${shiftId}`;
    planningOverviewEditingShiftKey = planningOverviewEditingShiftKey === shiftKey ? "" : shiftKey;
    renderSchedulePlanningOverview();
    return;
  }
...
approvalPreviousWeekButton?.addEventListener("click", () => {
  const previousWeek = getPreviousWeekValue(approvalWeekInput?.value || getCurrentWeekValue());
  if (approvalWeekInput) {
    approvalWeekInput.value = previousWeek;
  }
  if (hoursExportWeekInput) {
    hoursExportWeekInput.value = previousWeek;
  }
  renderHoursExportControls();
  renderHoursApproval();
});

approvalCurrentWeekButton?.addEventListener("click", () => {
  const currentWeek = getCurrentWeekValue();
  if (approvalWeekInput) {
    approvalWeekInput.value = currentWeek;
  }
...
approvalNextWeekButton?.addEventListener("click", () => {
  const nextWeek = getNextWeekValue(approvalWeekInput?.value || getCurrentWeekValue());
  if (approvalWeekInput) {
    approvalWeekInput.value = nextWeek;
  }
  if (hoursExportWeekInput) {
    hoursExportWeekInput.value = nextWeek;
  }
  renderHoursExportControls();
  renderHoursApproval();
});
